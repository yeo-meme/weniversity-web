import type {LocalChapterCache,WatchProgress,LocalProgressCache} from '../types/progress.types'
export function localChapterToWatchProgress(
  ...
  export function convertWatchProgressToCache(watchProgress: WatchProgress): LocalChapterCache {
    return {
      ...
      for (const chapterIdStr in chapters) {
        ...
        if (serverChapter) {
          ...
          const wp: WatchProgress = {
            ...
            completedAt: localChapter.isCompleted ? Date.now() : serverChapter.completedAt
          };
          result.push(wp);
        } else {
