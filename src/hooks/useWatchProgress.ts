...
export const useWatchProgress = (userId: string, chapterId: number) => {
  ...
  const loadOrCreateProgress = useCallback(() => {
    try {
      ...
      // 🎯 진행률 데이터가 없으면 새로 생성
      if (!progress) {
        progress = WatchProgressService.createWatchProgress({ userId, chapterId });
      }
  ...
  // 진행률 업데이트
  const updateProgress = useCallback((updates: UpdateWatchProgressParams) => {
    try {
      const updated = WatchProgressService.updateWatchProgress(userId, chapterId, updates);
      if (updated) {
